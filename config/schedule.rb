...
if !Dir.exist?("/home/ec2-user/Hobby/current/log/production/#{Time.now.strftime('%Y%m%d')}")
...
if rails_env.to_sym != :development
    ...
    every 1.day do
        begin
            ...
            command "/home/ec2-user/Hobby/current/bash.sh"
        rescue => e
    ...
    # every 1.week do
    #     begin
    #         command "/usr/bin/zip -r /home/ec2-user/Hobby/current/log/production/#{(Date.today - 2 ).strftime('%Y%m%d')}.zip /home/ec2-user/Hobby/current/log/production/#{(Date.today - 2 ).strftime('%Y%m%d')}", :environment_variable => "RAILS_ENV", :environment => "production"
    #     rescue => e
    ...
    every 8.day do
        begin
            ...
            command "/bin/rm -rf /home/ec2-user/Hobby/current/log/production/#{(Date.today - 8 ).strftime('%Y%m%d')}", :environment_variable => "RAILS_ENV", :environment => "production"
        rescue => e
            Rails.logger.error("aborted  command  delete dir")
            raise e
        end
    end
